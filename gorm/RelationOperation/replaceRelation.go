...
func ReplaceRelation () {
	...
	DB.Model(&a1).Association("Essays").Replace([]*Essay{&e1, &e2, &e1})
	DB.Model(&a1).Association("Essays").Replace([]*Essay{&e1})
	fmt.Println(len(a1.Essays))
	  ...
	  // replace 多对多会将不包括在 replace 列表里的记录清空
		DB.Model(&e1).Association("Tags").Append([]*Tag{&t1,&t3})
		DB.Model(&e1).Association("Tags").Replace([]*Tag{&t2})
		...
		// replace 一对一会将不包括在 replace 列表里的记录置为 NULL
		DB.Model(&e1).Association("EssayMate").Append(&am)
		DB.Model(&e1).Association("EssayMate").Replace(&am1)
